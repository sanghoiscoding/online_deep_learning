...
def test_logging(logger: tb.SummaryWriter):
    ...
    for epoch in range(10):
        ...
        train_acc_epoch = sum(metrics['train_acc']) / len(metrics['train_acc'])
        logger.add_scalar('train_accuracy', train_acc_epoch, global_step - 1)
        ...
        val_acc_epoch = sum(metrics['val_acc']) / len(metrics['val_acc'])
        logger.add_scalar('val_accuracy', val_acc_epoch, global_step - 1)
